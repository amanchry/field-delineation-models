"""
Evaluate the frozen base SAM2 (no fine-tuning) on AI4Boundaries test split.

Uses SAM2AutomaticMaskGenerator with the original SAM2 weights — no decoder
checkpoint is loaded. This is the zero-shot baseline for SAM2.
...
def main():
    device = best_device()
    print(f"Device: {device} | Variant: sam2_{VARIANT} | Frozen (zero-shot) | Data: {DATA_ROOT}")
    ...
    out_dir = ROOT / "sam2" / "outputs" / f"{VARIANT}_frozen"
    ...
    print_stats_table(rows, f"SAM2 Frozen ({VARIANT})", SPLIT)
    save_all_plots(rows, visuals, out_dir, f"SAM2 Frozen ({VARIANT})",
