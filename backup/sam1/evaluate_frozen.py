"""
Evaluate the frozen base SAM1 (no fine-tuning) on AI4Boundaries test split.

Uses SamAutomaticMaskGenerator with the original SAM1 weights — no decoder
checkpoint is loaded. This is the zero-shot baseline for SAM1.
...
OUT_ROOT   = f"sam1/outputs/{VARIANT}_frozen"
SPLIT       = "test"
CONF_THRESH = 0.5
N_VISUAL    = 12
...
def main():
    ...
    print(f"Device: {device} | Variant: sam1_{VARIANT} | Frozen (zero-shot) | Data: {DATA_ROOT}")
    ...
    print_stats_table(rows, f"SAM1 Frozen ({VARIANT})", SPLIT)
    save_all_plots(rows, visuals, out_dir, f"SAM1 Frozen ({VARIANT})",
